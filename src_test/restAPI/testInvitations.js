        ...
        before(
            async () => { 
                ...
                try{
                    ...
                    db.run(statement7, (err)=>{
                        debug("Add meeting invitation.");
                    });
                    db.run(statement8, (err)=>{
                        debug("Add meeting invitation.");
                    });
                    db.run(statement9, (err)=>{
                        debug("Add meeting invitation.");
                    });
                ...
                it('it should update the invitation with invitationId=1', 
                    (done) => {
                        let newData = {
                            ...
                            acceptedDatesAndTimes: [{startDate: new Date(), endDate: new Date()}, {startDate: new Date(), endDate: new Date()}],
                        }
                        chai.request(server)
                            .put('/invitations/1')
                            .send(newData)
